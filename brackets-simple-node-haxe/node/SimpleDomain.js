...
var Node = function() { }
Node.main = function() {
	Node.os = js.Node.require("os");
	exports.init = this.init;
}
Node.cmdGetMemory = function() {
	return { total : Node.os.totalmem(), free : Node.os.freemem()};
}
Node.init = function(domainManager) {
	if(!domainManager.hasDomain("simple")) domainManager.registerDomain("simple",{ major : 0, minor : 1});
	domainManager.registerCommand("simple","getMemory",Node.cmdGetMemory,false,"Returns the total and free memory on the user's system in bytes",[],[{ name : "memory", type : "{total: number, free: number}", description : "amount of total and free memory in bytes"}]);
}
var Std = function() { }
